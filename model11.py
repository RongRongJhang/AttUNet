...
class Denoiser(nn.Module):
    def __init__(self, num_filters=32, kernel_size=3, activation='relu'):
        ...
        # 編碼器
        self.enc_conv1_1 = nn.Conv2d(1, num_filters, kernel_size=kernel_size, padding=1)
        self.enc_conv1_2 = nn.Conv2d(num_filters, num_filters, kernel_size=kernel_size, padding=1)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        self.enc_conv2_1 = nn.Conv2d(num_filters, num_filters * 2, kernel_size=kernel_size, padding=1)
        self.enc_conv2_2 = nn.Conv2d(num_filters * 2, num_filters * 2, kernel_size=kernel_size, padding=1)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        self.enc_conv3_1 = nn.Conv2d(num_filters * 2, num_filters * 4, kernel_size=kernel_size, padding=1)
        self.enc_conv3_2 = nn.Conv2d(num_filters * 4, num_filters * 4, kernel_size=kernel_size, padding=1)
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        # 瓶頸層
        self.bottleneck = MultiHeadSelfAttention(embed_size=num_filters * 4, num_heads=4)
        
        # 解碼器
        self.up3 = nn.ConvTranspose2d(num_filters * 4, num_filters * 2, kernel_size=2, stride=2)
        self.dec_conv3_1 = nn.Conv2d(num_filters * 6, num_filters * 2, kernel_size=kernel_size, padding=1)  # 修正：96 -> 32
        self.dec_conv3_2 = nn.Conv2d(num_filters * 2, num_filters * 2, kernel_size=kernel_size, padding=1)
        
        self.up2 = nn.ConvTranspose2d(num_filters * 2, num_filters, kernel_size=2, stride=2)
        self.dec_conv2_1 = nn.Conv2d(num_filters * 3, num_filters, kernel_size=kernel_size, padding=1)  # 修正：48 -> 16
        self.dec_conv2_2 = nn.Conv2d(num_filters, num_filters, kernel_size=kernel_size, padding=1)
        
        self.up1 = nn.ConvTranspose2d(num_filters, num_filters, kernel_size=2, stride=2)
        self.dec_conv1_1 = nn.Conv2d(num_filters * 2, num_filters, kernel_size=kernel_size, padding=1)  # 正確：32 -> 16
        self.dec_conv1_2 = nn.Conv2d(num_filters, num_filters, kernel_size=kernel_size, padding=1)
        
        self.output_layer = nn.Conv2d(num_filters, 1, kernel_size=1)
    ...
    def forward(self, x):
        # 編碼器
        x1 = self.activation(self.enc_conv1_1(x))
        x1 = self.activation(self.enc_conv1_2(x1))
        p1 = self.pool1(x1)
        
        x2 = self.activation(self.enc_conv2_1(p1))
        x2 = self.activation(self.enc_conv2_2(x2))
        p2 = self.pool2(x2)
        
        x3 = self.activation(self.enc_conv3_1(p2))
        x3 = self.activation(self.enc_conv3_2(x3))
        p3 = self.pool3(x3)
        
        # 瓶頸層
        x = self.bottleneck(p3)
        
        # 解碼器
        x = self.up3(x)
        x = torch.cat([x, x3], dim=1)
        x = self.activation(self.dec_conv3_1(x))
        x = self.activation(self.dec_conv3_2(x))
        
        x = self.up2(x)
        x = torch.cat([x, x2], dim=1)
        x = self.activation(self.dec_conv2_1(x))
        x = self.activation(self.dec_conv2_2(x))
        
        x = self.up1(x)
        x = torch.cat([x, x1], dim=1)
        x = self.activation(self.dec_conv1_1(x))
        x = self.activation(self.dec_conv1_2(x))
