...
# model7_modify5_fix2 將 ShuffleAttention 改良
...
class ShuffleAttention(nn.Module):
    def __init__(self, channels, reduction=4, groups=4):
        ...
        self.channel_fc = nn.Sequential(
            nn.Linear(self.group_channels, self.group_channels // 2),
            nn.ReLU(),
            nn.Linear(self.group_channels // 2, self.group_channels),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        # Channel attention
        y = self.avg_pool(x_group).squeeze(-1).squeeze(-1)  # [b*g, group_c]
        y = self.channel_fc(y).view(b * self.groups, self.group_channels, 1, 1)
        
        # Spatial attention
        z = torch.cat([x_group.mean(dim=1, keepdim=True), 
                      x_group.max(dim=1, keepdim=True)[0]], dim=1)
        z = self.sigmoid(self.spatial_conv(z))
    ...
    def _init_weights(self):
        # Initialize channel_fc layers
        for m in self.channel_fc:
            if isinstance(m, nn.Linear):
                init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                if m.bias is not None:
                    init.constant_(m.bias, 0)
        # Initialize spatial_conv
        init.kaiming_normal_(self.spatial_conv.weight, mode='fan_out', nonlinearity='relu')
        if self.spatial_conv.bias is not None:
            init.constant_(self.spatial_conv.bias, 0)
